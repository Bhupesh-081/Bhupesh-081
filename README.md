<!-- ===================== -->
<!-- 👽 ALIEN GITHUB README -->
<!-- ===================== -->

<div align="center">

  <img src="https://readme-typing-svg.demolab.com?font=Orbitron&size=28&duration=2500&pause=500&color=00FFD1&center=true&vCenter=true&width=800&lines=FULL+STACK+ROBOTICS+ENGINEER;ROS2+%7C+GAZEBO+%7C+RVIZ2;AI+%7C+ML+%7C+AUTONOMOUS+SYSTEMS;WARNING%3A+NON-HUMAN+DEVELOPER" />

  <br><br>

  <img src="https://user-images.githubusercontent.com/74038190/212284068-0b9c6e3a-0a1c-4cfa-b8fd-6d6c5b1bb8f6.gif" width="500"/>

</div>

---

<h1 align="center">👾 Hi, I'm Bhupesh — Robotics isn’t my skill, it’s my habitat</h1>

<h3 align="center">
🚀 Full Stack Robotics Engineer | 🤖 Autonomous Systems | 🧠 AI + ML | 🛰 ROS2 Architect
</h3>

---

## 🧬 About Me (Decoded Transmission)

<ul>
  <li>🧠 I design robots that <b>think, see, and decide</b></li>
  <li>🛰 ROS2 native — not migrated, <i>born</i> into it</li>
  <li>🤖 Simulate first, deploy later (Gazebo + RViz2 supremacy)</li>
  <li>🧪 Mixing AI, ML, and Robotics like forbidden chemistry</li>
  <li>🌌 Building systems for Earth, Moon, and competitions like <b>ABU Robocon</b></li>
</ul>

---

## ⚙️ Tech Stack from Another Planet

### 🤖 Robotics & Middleware
<p>
  <img src="https://img.shields.io/badge/ROS2-0A0FF9?style=for-the-badge&logo=ros&logoColor=white"/>
  <img src="https://img.shields.io/badge/Gazebo-000000?style=for-the-badge&logo=gazebo&logoColor=white"/>
  <img src="https://img.shields.io/badge/RViz2-FF6F00?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/micro--ROS-2E77BC?style=for-the-badge"/>
</p>

### 🧠 AI / ML / Computer Vision
<p>
  <img src="https://img.shields.io/badge/Machine%20Learning-102230?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Deep%20Learning-5B2EFF?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white"/>
  <img src="https://img.shields.io/badge/Computer%20Vision-00FFAA?style=for-the-badge"/>
</p>

### 💻 Programming Languages
<p>
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus"/>
  <img src="https://img.shields.io/badge/Python-FFD43B?style=for-the-badge&logo=python"/>
  <img src="https://img.shields.io/badge/Embedded%20C-2C2C2C?style=for-the-badge"/>
</p>

### 🔌 Embedded & Hardware
<p>
  <img src="https://img.shields.io/badge/ESP32-000000?style=for-the-badge&logo=espressif"/>
  <img src="https://img.shields.io/badge/STM32-03234B?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Raspberry%20Pi-A22846?style=for-the-badge&logo=raspberrypi"/>
  <img src="https://img.shields.io/badge/Jetson-76B900?style=for-the-badge&logo=nvidia"/>
</p>

---

## 🧪 Simulation + Control Arsenal

<ul>
  <li>⚙️ URDF / XACRO modeling</li>
  <li>🧭 SLAM (2D & 3D)</li>
  <li>🎯 Navigation2 Stack</li>
  <li>📡 Sensor Fusion (IMU + LiDAR + Camera)</li>
  <li>🎮 Teleoperation & Autonomous Modes</li>
</ul>

---

## 🧠 Things I Build While Humans Sleep

<ul>
  <li>🤖 Autonomous Rovers & Mobile Robots</li>
  <li>🛰 Lunar & Space Habitat Simulations</li>
  <li>🧪 AI-powered Perception Pipelines</li>
  <li>🕹 ROS2 Teleop Systems</li>
  <li>📊 Custom Dashboards & GUIs</li>
</ul>

---

## 📊 GitHub Stats (Reality Distortion Field)

<div align="center">

  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&theme=radical&hide_border=true" height="180"/>

  <img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_GITHUB_USERNAME&theme=radical&hide_border=true" height="180"/>

</div>

---

## 🧭 Current Mission

```text
✓ Build robots that survive chaos
✓ Teach machines to understand reality
✓ Compete at national & international levels
✓ Push ROS2 beyond documentation
✓ Blur the line between software and hardware
